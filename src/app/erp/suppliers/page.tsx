...
export default function SuppliersPage() {
  ...
  const loadEntities = async (token: string) => {
    setLoading(true);
    try {
      const response = await fetch(`/api/entities?type=supplier&page=${page}&limit=20&search=${search}`, {
  ...
  const menuItems = [
    ...
    { icon: '🏢', label: 'Fornecedores', href: '/erp/suppliers', active: true },
    ...
    <div className="min-h-screen flex">
      ...
      <main className="flex-1 p-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <a href="/erp/suppliers/new" className="btn-primary">
            + Novo Fornecedor
          </a>
        </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="text-left py-3 px-4 text-sm font-semibold text-gray-600">IE/IM</th>
                  ...
                  {entities.map((entity) => (
                    <tr key={entity.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4 text-sm text-gray-600">
                        {entity.stateRegistration || entity.municipalRegistration || '-'}
                      </td>
                      ...
                      <td className="py-3 px-4">
                        <a href={`/erp/suppliers/${entity.id}`} className="text-primary hover:underline text-sm">
                          Ver
                        </a>
                      </td>
                    </tr>
